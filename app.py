...
def admin_media():
 if request.method=='POST':
  ...
  if f and f.filename and f.filename.rsplit('.',1)[-1].lower() in {'png','jpg','jpeg','webp','gif'}:
   ext=f.filename.rsplit('.',1)[-1].lower(); fn=secure_filename(f.filename.rsplit('.',1)[0])+'_'+uuid.uuid4().hex[:8]+'.'+ext; f.save(UP/fn); flash('تصویر آپلود شد.','ok')
 files=[]
 for p in sorted(UP.iterdir(),key=lambda x:x.stat().st_mtime,reverse=True) if UP.exists() else []:
