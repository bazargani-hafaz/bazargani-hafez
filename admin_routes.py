...
def register_admin_routes(app):
    ...
    def admin_products():
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                colors=request.form.get('colors','').strip(); price=0
    ...
    def admin_product_edit(pid):
        ...
        if request.method=='POST':
            ...
            c.execute('UPDATE products SET name=?,slug=?,category_id=?,brand=?,model=?,sku=?,colors=?,stock=?,material=?,warranty=?,image=?,gallery=? WHERE id=?',(name,unique_slug(c,name,pid),request.form.get('category_id') or None,request.form.get('brand','').strip(),request.form.get('model','').strip(),request.form.get('sku','').strip(),request.form.get('colors','').strip(),int(request.form.get('stock') or 0),request.form.get('material','').strip(),request.form.get('warranty','').strip(),image,gallery,pid)); c.commit(); c.close(); flash('محصول ویرایش شد.','success'); return redirect(url_for('admin_products'))
    ...
    def admin_settings():
        ...
        settings={r['key']:r['value'] for r in c.execute('SELECT * FROM settings')}; c.close()
